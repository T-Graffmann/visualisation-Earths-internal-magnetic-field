function main(Nmin,Nmax,r,phistep,thetastep,Year,Month,DayOfMonth,Model)
...
[differentiator,loadedTable,loadedTable2]=ChooseCHAOSorIGRF(Nmin,Nmax,Model);
    ...
    %   define theta for validation
    Theta=linspace(pi/100,pi-pi/100,thetastep);
    %   define theta for general use
    %Theta =linspace(pi/thetastep,pi-pi/thetastep,thetastep);%  poles are cut off because of division by 0 at poles
    ...
    % height %
    height=r-a;
    strHeight=append("H",string(height));

    % date %
    strDate=append("Y",string(Year),"M",string(Month),"D",string(DayOfMonth));

    % data type + file names array%
    strDATA=["Bx" "By" "Bz" "Bhor" "Btotal" "Ic" "Dc" "DatLat" "DatLon"];
    strDATA=append(strDate,strDATA,strHeight);
    filenames=append(strDATA,'.txt');
    
    % safe data %
    writematrix(BxF,filenames(1));
    writematrix(ByF,filenames(2));
    writematrix(BzF,filenames(3));
    writematrix(Bhor,filenames(4));
    writematrix(Btotal,filenames(5));
    writematrix(Ic,filenames(6));
    writematrix(Dc,filenames(7));
    writematrix(YLat,filenames(8));
    writematrix(XLon,filenames(9));
